...
BEGIN
  ...
  -- 时间范围筛选 - 移除remark匹配要求，直接进行日期范围筛选
  -- 日期范围筛选 - 筛选包含在所选时间范围内的数据
...
-- 验证函数是否成功创建
SELECT '函数部署完成！' as deployment_status;
SELECT proname as function_name, pronargs as parameter_count FROM pg_proc WHERE proname = 'get_employee_join_data';
